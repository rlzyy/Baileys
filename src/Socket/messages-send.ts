/**
 * update send message (support channel - newsletter)
*/
...
export const makeMessagesSocket = (config: SocketConfig) => {
  ...
  * */
  ...
  const createParticipantNodes = async(
    ...
    return {
      nodes, shouldIncludeDeviceIdentity
    }
  }
  ...
  const relayMessage = async(
    ...
    message: proto.IMessage,
    {
      messageId: msgId, participant, additionalAttributes, useUserDevicesCache, useCachedGroupMetadata, statusJidList
    }: MessageRelayOptions
  ) => {
    ...
    await authState.keys.transaction(
      async() => {
        ...
        if (isGroup || isStatus) {
          const [groupData,
            senderKeyMap] = await Promise.all([
              (async() => {
          ...
          const [{
          ...
          },
            {
              ...
              createParticipantNodes(meJids, meMsg, mediaType ? {
                mediatype: mediaType
              }: undefined),
              createParticipantNodes(otherJids, message, mediaType ? {
                mediatype: mediaType
              }: undefined)
            ])
    ...
    sendMessage: async(
      ...
      if (
        ...
        await relayMessage(jid, fullMsg.message!, {
          messageId: fullMsg.key.id!, useCachedGroupMetadata: options.useCachedGroupMetadata, additionalAttributes, statusJidList: options.statusJidList
        })
